...
import ReviewalsItem from 'src/components/item/ReviewalsItem';
import ReviewalsAction from 'src/components/use/ReviewalsAction';
import ReviewalsPrompt from 'src/components/prompt/ReviewalsPrompt';

const ReviewalsPanel: React.FC = () => {
  ...
  const MemoizedRequestItem = React.memo(({ item }: { item: SchemaRequestApplications }) => (
    <ReviewalsItem onPanel={state.selectedButton} data={item} navigation={navigation} />
  ));
    ...
    <React.Fragment>
      ...
      {state.failedList!.length > 0 ? (
        ...
        <SuccessPrompt
          ...
          subTitle={STRINGS.successReviewals(handle.isAction as number, state.count!)}
