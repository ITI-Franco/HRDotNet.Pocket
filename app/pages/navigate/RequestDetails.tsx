...
const RequestDetails: React.FC<TypeNavStack> = ({ navigation }) => {
  ...
  const DisplayApprovalButton = () => {
    ...
    const onCheckFiled = () => {
      return filingStatusId === FilingStatus.Filed;
    };
      ...
      <View style={{ flexDirection: 'row' }}>
        ...
        <TouchableOpacity
          style={[
            ...
            {
              backgroundColor: onCheckFiled() ? COLORS.purple : COLORS.green,
            },
          ]}
          onPress={onHandleApprove}
        >
          <FontAwesome name={onCheckFiled() ? 'search' : 'check-circle'} size={24} color={COLORS.clearWhite} />
          <Text style={styles.textButton}>{onCheckFiled() ? STRINGS.review : STRINGS.approve}</Text>
        </TouchableOpacity>
      </View>
    );
  };
  ...
  useEffect(() => {
    const timeoutId = setTimeout(async () => {
      await useFetch.RequestById(navigation, state, setState, handle, setHandle);
    }, 500);
